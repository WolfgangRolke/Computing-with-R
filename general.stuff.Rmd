...
options(digits=5)
pi
options(digits=15) #reset to default!
```
...
the ^ in front means that *ls* will list all object whose name starts with sa. This is what is called a *regular expression*. These are general (non R) rules for specifying patters. They are a science all their own, and we will need to have a look t them at some point.
...
Often you create an object that is only need for a short time. If you want to get rid of it use *rm* or (remove)
