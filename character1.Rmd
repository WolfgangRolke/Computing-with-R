...
2)  for  regular  expression operations
...
next str_length. Compared to nchar(), can handle more data types, for example factors:
...
Another handy function that we can find in stringr is str_pad() for padding a string. This is useful if we want to have a nice alignmend when printing some text.
...
One operation that one needs to do quite often is to extract the last (few) letters from words. Using substring is tricky if the words don't have the same lengths:
...
Actually, time is the most common word in both books!

Can we do a graphical display of the word frequencies? We will need some rutines from yet another package called *tidyr*.

We begin by joining the two book together, with a new column identifying the book:
...
```{r}
freqs %>% 
   spread(key=book, value = prop) %>% #use one column for Dracula's 
                #proportions and another for Time Machine
   na.omit()  -> #words that appear in only one book are NA, 
                #eliminate them    
  common.words
print(common.words, n=4)
ggplot(common.words, aes(x=Dracula, 
                         y= common.words[ ,"Time Machine"])) +
  labs(x="Dracula", y="Time Machine") +
