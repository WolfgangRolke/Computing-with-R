...
The "\\n" (**n**ewline) is needed so that the cursor moves to the next line. 
...
Here the f stands for floating point, the most common type. Also note that the result if a call to sprintf is a character vector.
...
-  use the argument sep=";" to change the symbol that is being use as a separator. The default is empty space, common cases include  comma, semi-colon, and newline (\\n)  
