...
tmpR <- round(
  summary(microbenchmark(dist2(x, y)))["mean"], 1)
tmpR
```
...
```{r}
tmpRcpp <- round(
  summary(microbenchmark(dist4(x, y)))["mean"], 1)
tmpRcpp
```
...
Now the difference between `r tmpRcpp` and `r tmpR`  milliseconds (the fastest non-C++ time) might not seem like much, but imagine if we had to run this routine one million times, then the R routine would take `r tmpR` $*10^{-3}\times 10^6/3600=$ `r round(tmpR*10^{-3}*10^6/3600,1)` hours, but the Cpp routine takes only `r round(tmpRcpp*10^{-3}*10^6/3600, 1)` hours. Writing it takes literally 1 minute! (assuming you know how)  
