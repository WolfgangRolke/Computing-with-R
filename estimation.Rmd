...
The problem is to find an estimate of $\theta$ based on the data $X_1,..,X_n$, that is a function (called a *statistc*) $T(X_1,..,X_n)$ such that in some sense $T(X_1,..,X_n) \approx \theta$.
...
One major reason for the popularity of this method is the following celebrated theorem, due to Fisher: under some regularity conditions 
...
$$
where $z_{\alpha/2}$ is a quantile from the normal distribution found with
...
$$
this is of course the standard answer (for known $\sigma=1$).

###Example

$X_1,..,X_n \sim B (\alpha, \alpha)$. 
...
so finding the mle analytically would require us to find the derivative of $\log \Gamma (\alpha)$, which is impossible. We will have to do this numerically.
...
Notice that the Fisher Information is the expected value of the Hessian matrix, and by the theorem of large numbers $\frac1{n} \sum H \rightarrow I$. Now if we just replace $I$ with observed information we get:
...
```{r}
mle.est <- function(f, param, alpha=0.05, rg, do.graph=FALSE) {
  ll <- function(a) {
     -sum(log(f(a)))
  } 
...
```{r}
mle.est(function(a) {dbeta(x, a, a)}, param=2.5, 
        rg=c(1, 5), do.graph = TRUE)
```
...
mle.est(function(a) {dnorm(x, a)},  param=10,
        rg=c(5, 15), do.graph = TRUE)
```
