...
-\frac1{2\sigma^2} (x-\mu)^2\right\} \frac{1}{\sqrt{2\pi b^2}} \exp \left\{ -\frac1{2 b^2} (\mu-a)^2\right\}  d\mu \\
\end{aligned}
$$
...
$$
\begin{aligned}
&m(x)  = K_4 \int_{-\infty}^\infty \exp \left\{ 
-\frac12 \frac{(\mu-d)^2}{c^2}  \right\} d \mu= K_5 \\
...
&f(\theta| \textbf{x}) = f(\textbf{x}|\theta)\pi(\theta)/m(\textbf{x})    = \\
& K_6 \exp \left\{ 
-\frac12 \frac{(\mu-d)^2}{c^2}  \right\}  \\
\end{aligned}
$$
Notice that we don't need to worry about what exactly $K_6$ is, because the posterior will be a proper probabilityt density, so $K_6$ will be what it has to be!So we found
...
$$
\begin{aligned}
&m(x)  = \int_{-\infty}^\infty \frac{1}{\sqrt{2\pi \sigma^2}} \exp \left\{ 
-\frac1{2\sigma^2} (x-\mu)^2\right\} 
\frac1{\Gamma(\alpha)\beta^\alpha}\mu^{\alpha-1}\exp{(-\mu/\beta)}
d\mu 
\end{aligned}
$$
...
```{r}
fmu <- function(mu) 
  dnorm(mean(dta.norm), mu, 2.3/sqrt(20))*dgamma(mu, 5, 1)
mx <- integrate(fmu, lower=0, upper=Inf)$value
mu.f <- function(mu) fmu(mu)/mx
integrate(function(mu) {mu*mu.f(mu)}, lower=0, upper=Inf)$value
```
...
where $F$ is the posterior distribution function. Again we need to work numerically:

```{r}
mu.F <- function(t) integrate(mu.f, lower=0, upper=t)$value 
t <- 0
repeat {
  t <- t+0.01
  if(mu.F(t)>0.025) break
}
left <- t
repeat {
  t <- t+0.01
  if(mu.F(t)>0.975) break
}
c(left, t)
```
...
```{r}
bayes.credint <- function(x, df, prior, conf.level=0.95,
          step=0.01, lower=-Inf, upper=Inf,  Show=TRUE, rg) {
  ...
  repeat {
      t <- t+step
      if(pF(t, x)>(1-conf.level)/2) break
  }
  left <- t
  t <- upper
  repeat {
      t <- t-step
      if(pF(t, x)<1-(1-conf.level)/2) break
  }
  if(Show) abline(v=c(left, t))
  c(parhat, left, t)
}
```
...
bayes.credint(mean(dta.norm), df=df, prior=prior,
      lower=2, upper=9, Show=T, rg=c(4,7))
```
...
bayes.credint(x=31, df=df, prior=prior, step=0.001, conf.level = 0.9,
      lower=0, upper=1, Show=T, rg=c(0.1, 0.4))
```
...
bayes.credint(x=578, df=df, prior=prior, step=0.001,
      lower=0, upper=1, Show=T, rg=c(0.5, 0.65))
```
...
bayes.credint(dta.beta1, df=df, prior=prior,
      lower=1.5, upper=2.5, Show=T, rg=c(1.5, 2.5))
```
