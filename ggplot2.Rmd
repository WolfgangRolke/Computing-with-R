...
In 2005 Wilkinson, Anand, and Grossman published the book "The Grammar of Graphics". In it he layed out a systematic way to describe any graph in terms of basic building blocks. ggplot2 is an implementation of his ideas.

The use of the grammar seems a bit strange here. The general meaning of the word grammar is: 
...
Here we have two quantitative variables, and so the obvious thing to do is a scatterplot. But there are a number of different ways we can this:

```{r echo=3:7, warning=FALSE}
attach(mtcars)
par(mfrow=c(2, 2))
...
The basic idea of the grammar of graphs is to separate out the parts of the graphs: there is the basic layout, there is the data that goes into it, there is the way in which the data is displayed. Finally there are annotations, here the titles, and possibly more things added, such as a fitted line. In ggplot2 you can always change one of these without changing any of the others.

Another way of looking at it this: in ggplot2 you build a graph like a lasagne: layer by layer.

Take the graph on the lower right. Here I made the plotting symbol bigger (with cex=2). But now the graph doesn't look nice any more, the first and the last circle don't fit into the graph. The only way to fix this is to start all over again, by making the margins bigger:

```{r}
plot(hp, mpg, cex=2, ylim=range(mpg)+c(-1, 1))
```

and that is a bit of work because I have to figure out how to change the margins. In ggplot2 that sort of thing is taken car of automatically!

Let's start by recreating the first graph above. We first have to turn the data into a data frame, the basic data structure for ggplots
...
-  it's first argument is the data set, as a data frame  
...
This seems much easier at first (and it is) but the qplot command is also very limitted. Very quickly you want to do things that aren't possible with qplot, and so I won't dsicuss it further here.
...
Notice that the legend is a continous color scale. This is because the variable cy has values 4, 6, and 8, and so is identified as a numeric variable. In reality it is categorical (ever seen a car with 1.7 cylinders?), and so we should change that:


```{r}
mtcars$faccyl <- factor(cyl, levels = c(4, 6, 8), ordered = TRUE) 
ggplot(mtcars, aes(hp, mpg, color=faccyl)) + geom_point()
```
...
```{r }
ggplot(mtcars, aes(hp, mpg, shape=faccyl)) + geom_point()
```

or both:

```{r}
ggplot(mtcars, aes(hp, mpg, shape=faccyl, color=faccyl)) + geom_point()
```
...
```{r}
plt1 <- ggplot(mtcars, aes(hp, mpg, color=faccyl)) + geom_point() 
plt2 <- plt1 +
...
Say we want to add the least squares regression lines for each gender:

```{r}
plt3 <- plt2 + geom_smooth(method = "lm", se=FALSE)
plt3
```
...
```{r}
mtcars$facgear <- factor(gear, levels = 3:5, ordered = TRUE)
plt4 <- ggplot(aes(hp, mpg, color=faccyl), data = mtcars) + geom_point(size = 1) 
plt4 <- plt4 + facet_wrap(~facgear) 
plt4 <- plt4 +
  labs(x="Horsepower", y="Miles per Gallon", color="Cylinders") +
  labs(title="Milage goes down as Horsepower goes up")
plt4 <- plt4 + geom_smooth(method="lm", se=FALSE)
plt4
```
...
```{r message=FALSE}
df5 <- data.frame(x=c(rnorm(100, 10, 3), rnorm(80, 12, 3)), 
...
ggplot(df6, aes(x, y)) + geom_boxplot()
```

strangly enough doing a boxplot without groups takes a little work:

```{r}
df6$z <- rep(" ", length(y))
ggplot(df6, aes(z, y)) + geom_boxplot() + xlab("")
```

There is a modern version of this graph called a violin plot:

```{r}
ggplot(df6, aes(z, y)) + geom_violin() + xlab("")
```

###Barcharts

```{r}
x <- sample(LETTERS[1:5], size=1000, replace=TRUE, prob = 6:10)
df7 <- data.frame(x=x)
ggplot(df7, aes(x)) + 
...
```{r}
df7$y <- sample(c("X", "Y"), size=1000, replace=TRUE, prob=2:3)
...
One issue is figure sizing. You need to do this so that graph looks "good". Unfortunately this depends on where it ends up. A graph that looks good on a webpage might look ugly in a pdf. So it is hard to give any general guidelines.

If you use R markdown, a good place to start is with the chunk arguments
fig.with=6 and out.width="70%". In fact on top of every R markdown file I have a chunk with
