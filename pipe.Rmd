...
df <- mtcars
df1 <- subset(df, hp>70)
ggplot(data=df1, aes(disp, mpg))+geom_point()
```

This is not how we think, though. That would go somehing like this:
...
The basic package to use the piping is
...
mtcars %>%
  subset(x=., hp>70) %>%
  ggplot(data=., aes(disp, mpg))+geom_point()
```
...
written by Hadley Wickham. In essence it is a replacement for the *apply* famility of R routines. We can also write the above with
...
Notice how *filter* is aware of the pipe, it doesn't need to be told that it is supposed to work with mtcars. Sofar, *ggplot* is not, but this will change in the near future.

##tibbles

data frames have been the main data format of R since its beginnings, and are likely to stay that way for a long time. They do, however have some shortcomings. Among other things, when you type the name of a data frame and hit enter, all of it is shown, even if the data set is huge. On the other hand, interesting information such as the data types of the columns is not shown. To eliviate these (and some other) issues the data format *tibble* was invented. We can turn a data frame into a tibble with
...
If you create a tibble from scratch use:
...
We have already seen the filter command, the dplyr version of subset. Here the most important other dplyr commands: 
...
babynames %>%
    ...
    ggplot(data = ., aes(year, total, color = sex)) +
      geom_line() + labs(color="Gender") +
      ggtitle('Names starting with "W"') 
```

How often is my names used for a baby in the US?
...
babynames %>%              #take babynames
    ...
    cbind()               # then turn data around for easier 
                          #  reading      
```
...
This unfortunately breaks the logic of piping. There is a better way, though. Just remember that the logic of the assignment character <-, it's an arrow!
...
Let's say we want to find the times out of 100000 that the most popular names occured in the 2015:
...
They are all centered around *tidy data*. This is defined as data were
...
One big difference between data frames and tibbles is that tibbles automatically ignore row names:
